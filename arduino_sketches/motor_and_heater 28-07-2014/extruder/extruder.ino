...
// Extruder variables
float screwSpeed = 1;
double temperature1 = 2;
float temperature2 = 3;

// Setpoint values
float screwSpeedSetpoint = 0;
double temperature1Setpoint = 50;
...
void setup(){
  ...
  pinMode(1, OUTPUT); //IN1 pin, controls the black terminal of the motor (GND)
  pinMode(0, INPUT); //SF flag
  
  pinMode(13, OUTPUT); //emergency LED pin
  
  pinMode(12, OUTPUT); //Heater PWM output
  
  heaterPID.SetOutputLimits(0.0, 1.0);
...
void loop(){
  ...
  //read temperature
  temperature_ADC_lecture = analogRead(2);
  temperature1 = temperature_lut[temperature_ADC_lecture];
  
  lcd.setCursor(0,0);
  lcd.print(temperature1);
  lcd.setCursor(9,0);
  lcd.print(temperature1Setpoint);
  lcd.setCursor(0,1);
  lcd.print(screwSpeed);
  lcd.setCursor(9,1);
  //lcd.print(motor_duty_cycle);
  lcd.print(heater_duty_cycle);
  ...
  heaterPID.Compute(); //Only calculates PID parameters every Ts period (not every loop)
  
  analogWrite(12, heater_duty_cycle*255.0);
